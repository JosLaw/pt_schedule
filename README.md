...
The general app purpose is printed under the banner on page load. Several instructions are printed throughout the app with tailored error messages for user input. This is to ensure expectations of the site are met. The instructions also act as a clear guide to enhance UX and overall functionality of app
...
| Chose day. User input empty or not listed number | App crashed | No | Amend error parameters to include isnumeric() function and move int()
| Check worksheet. User input empty or not listed number | App crashed | No | Amend error parameters to include isnumeric() function and move int()
| Check worksheet. User input empty or not listed number | App crashed. Cant add int to str | No | Amend error parameters to include isnumeric() function and move int()


The deployed site was shared amongst family and friends to obtain user experience feedback. Overall the feedback was positive. There was some confusion in relation to the name format. To make it clearer the example has been amended in order to be more noticable. They were able to follow the rest of the instructions to create a profile and booking successfully.
...
* Gspread - Used to access and manipulate data from Google Worksheet
* Numpy - Used for multi-dimensional array in check_worksheet function to find free timeslots on spreadsheet
* Google sheets API - API to allow app to access and work with google worksheet
